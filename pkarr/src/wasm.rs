...
impl Client {
    ...
    pub async fn publish(&self, signed_packet: &SignedPacket, cas_timestamp: Option<f64>) -> Result<(), JsValue> {
        #[cfg(feature = "relays")]
        {
            if let Some(relays) = &self.relays {
                let cas = cas_timestamp.map(|ts| crate::Timestamp::from(ts as u64));
                
                relays.publish(signed_packet, cas)
                    .await
    ...
    pub async fn resolve_most_recent(&self, public_key_str: &str) -> Result<Option<SignedPacket>, JsValue> {
