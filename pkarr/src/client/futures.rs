...
/// Publish to both DHT and configured Relays.
///
/// Publish to the DHT first, if successful, publish to configured relays
/// in the background. If not await publishing to configured relays.
pub async fn publish_both_networks(
    ...
    match dht_future.await {
        Ok(_) => {
            tokio::spawn(relays_future);
            Ok(())
        }
        Err(_) => relays_future.await,
    }
}
