...
pub(super) async fn publish(
    ...
    let relay_publish = relays.publish(signed_packet, cas);
    let dht_publish = dht.publish(signed_packet, cas);
    let (relay_result, dht_result) = tokio::join!(relay_publish, dht_publish);

    let mut results = PublishResults::default();
    results.record(relay_result);
    results.record(dht_result);
    results.finish()
}
...
pub(super) async fn resolve(
    ...
    let r1 = relays.resolve(public_key, policy, more_recent_than);
    let r2 = dht.resolve(public_key, policy, more_recent_than);
